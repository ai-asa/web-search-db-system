...
3. 生命保険商品との関連性
   - 記事の内容が生命保険商品（死亡保障、医療保険、がん保険、介護保険、年金保険、学資保険など）と直接的に関連しているか
   - 顧客の生活保障や将来の経済的リスクに関する具体的な課題が含まれているか
   - 生命保険による解決や対策が明確に想定できる内容か
   - 生命保険の話題への展開が不自然でない、強引でないか
...
<analysis>
{
    ...
    "insurance_relevance": {
        ...
        "reasoning": "いずれかの生命保険商品を直接的な解決策として訴求できるかどうかの詳細な理由説明"
    }
}
</analysis>
...
- 生命保険商品との関連性は、無理なこじつけを避け、直接的な関連性のみを認めてください
...
def get_relevance_validation_prompt():
    """
    生命保険商品との関連性の妥当性を検証するためのプロンプト
    
    Returns:
        str: 関連性検証用プロンプト
    """
    return """あなたは生命保険営業における話題の適切性を判断する専門家です。
提供された情報と生命保険商品との関連性について、論理の飛躍やこじつけがないか厳密に検証してください。
...
{{
    ...
    "target_customers": "（Falseの場合は空文字）この話題が特に効果的な顧客層の具体的な特徴"
}}
</validation>

判定基準：
- is_valid = true の条件：
  1. 話題から生命保険の話題への展開が完全に自然であること
  2. 顧客の具体的な状況や生命保険に関するニーズと直接的に結びつくこと
  3. 生命保険による解決策が明確で説得力があること
  上記のすべてを満たす場合のみtrueとしてください

以下の場合は必ずfalseとしてください：
1. 話題と生命保険の結びつきに少しでも無理がある場合
2. 顧客の状況や生命保険に関するニーズとの関連が間接的な場合
3. 生命保険による解決策が抽象的な場合
